<?php
    ...
    use Dart\Response as Response;
    use Dart\Request as Request;
    ...
    class Application extends Container {
        ...
        protected $_paramTypes = [];
        protected $_callbacks = [ 'path' => [], 'param' => [], 'method' => [], 'subdomain' => [], 'domain' => [], 'format' => [], 'custom' => [] ];
        ...
        public function __construct(array $values = []) {
            ...
            // True = "1", "true", "on", "yes", False = "0", "false", "off", "no"
            ...
            if(is_array($types)) {
                foreach($this->_callbacks as $type => $stack) 
                    if(in_array($type, $types, true))
                        $this->_callbacks[$type][self::$_pathLevel] = [];
                return true;
            }

            $this->_callbacks = [ 'path' => [], 'param' => [], 'method' => [], 'subdomain' => [], 'domain' => [], 'format' => [], 'custom' => []];
        }

        public function path($path, \Closure $callback) {
            foreach( (array) $path as $step )
                $this->_callbacks['path'][self::$_pathLevel][trim($step, '/')] = $this->_prepClosure($callback);
            return $this;
        }
        ...
        protected function _prepClosure(\Closure $closure) {
            return $closure = $closure->bindTo($this);
        }

        protected function _push() {
            $this->_stack[] = [ $this->_request, $this->_response, $this->_paths, self::$_pathLevel, $this->_requestMethod, $this->_requestPath, $this->_currentPath, $this->_paramTypes, $this->_callbacks, $this->_helpers, $this->_responseHandlers
            ];
        }

        protected function _pop() {
            list( $this->_request, $this->_response, $this->_paths, self::$_pathLevel, $this->_requestMethod, $this->_requestPath, $this->_currentPath, $this->_paramTypes, $this->_callbacks, $this->_helpers, $this->_responseHandlers
         ...
         *
         * @param string|object $method HTTP request method string or Request object
        ...
        public function run($method = null, $uri = null) {
            ...
            if ($this->_nestingLevel <= 0) {
                /* We're the most outer run(). Save the _callbacks as our original callback setup */
                $this->_originalCallbacks = $this->_callbacks;
            } else {
                /* We were called from a handler callback. Reset _callbacks to the original*/
        ...
        protected function _runPath($method, $path, \Closure $callback = null) {
            ...
            if($this->isRequestPath() && isset($this->_callbacks['format'][self::$_pathLevel]) && count($this->_callbacks['format'][
        ...
        public function response() {
            //setter
            if( func_num_args() >= 2 && list($statusCode, $content) = func_get_args())
                $this->_response = $this->responseFactory($content, $statusCode);

            if(func_num_args() == 1)
                $this->_response = $this->responseFactory(func_get_arg(0));
        ...
        protected function responseFactory($content = null, $statusCode = null ) {
            ...
            if( $statusCode === null ){
                ...
                if($content === false) {
                    ...
                    $response = new \Dart\Response(null, 404);
                    $response->content($response->statusText(404));

                } elseif(is_int($content)) {
                    // Assume int response is desired HTTP status code
                    $response = new \Dart\Response(null, $content);
                    $response->content($response->statusText($content));
                } 

                // Convert response to Bullet\Response object if not one already
                if(!($response instanceof \Dart\Response))
                    $response = new \Dart\Response($content, 200);

            } elseif(!($content instanceof \Dart\Response)){
                $response = new \Dart\Response($content, $statusCode);
            }

            // Ensure no response body is sent for special status codes or for HEAD requests
            if(in_array($response->status(), [204, 205, 304]) || $this->request()->method() === 'HEAD')
                $response->content('');
        ...
        public function request() {
            return $this->_request === null ? $this->_request : new Request();
        }
        ...
        public function template($name, array $params = [] ) {
            return ( new View\Template($name))->set($params);
        }
        ...
        public function helper($name, $class = null) {
            if($class !== null)
                return $this->_helpers[$name] = $class && false;
         ...
         * returned by \Dart\App::run. Each callback is given the
         * \Dart\Response object as a parameter.
         *
        ...
        public function registerResponseHandler( $condition, $handler, $name = null ) {
            ...
            $handler = [ 'condition' => $condition, 'handler' => $handler ];
